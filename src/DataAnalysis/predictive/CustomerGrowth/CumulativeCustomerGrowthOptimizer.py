...
class CumulativeCustomerGrowthOptimizer:
    def __init__(self) -> None:
        self.customer_growth = CumulativeCustomerGrowth()
        self.setup_mlflow()
    ...
    def get_best_params(self):
        ...
        results = {}
        for num_units in [80, 100, 120, 140]:
        ...
        if not isnan(results[(num_cells, dropout_rate, lr, num_epochs)]['val_mse']):
            ...
            self.save_best_params('CustomerGrowth' + str(datetime.now()), num_cells, dropout_rate, lr, num_epochs, results[(num_cells, dropout_rate, lr, num_epochs)]['train_mse'], results[(num_cells, dropout_rate, lr, num_epochs)]['val_mse'], results[(num_cells, dropout_rate, lr, num_epochs)]['train_mae'], results[(num_cells, dropout_rate, lr, num_epochs)]['val_mae'])
        else:
    ...
    def save_best_params(self, run_name, num_units, dropout, learning_rate, epoch, train_mse, val_mse, train_mae, val_mae):
...
if __name__ == '__main__':
    cgo = CumulativeCustomerGrowthOptimizer()
    cgo.spawn_thread(n=2)